...
public class BluetoothPrintPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, RequestPermissionsResultListener {
    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        switch (call.method) {
            ...
            case "printTest":
                ...
                break;
            case "printerModel":
                result.success(DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getCurrentPrinterCommand() == PrinterCommand.ESC ? "ESC" : "TSC");
                break;
            case "changeModel":
                String model = args.get("name") + "";
                Log.i(TAG, "onMethodCall: changeModel:"+model +" TSC:"+(model.equals("TSC")) +" ESC："+(model.equals("ESC")));
                if (model.equals("ESC")) {
                    DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].sendByteDataImmediately(FactoryCommand.changPrinterMode(FactoryCommand.printerMode.ESC));
                } else if (model.equals("TSC")) {
                    DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].sendByteDataImmediately(FactoryCommand.changPrinterMode(FactoryCommand.printerMode.TSC));
                }
                result.success(null);
                break;
            default:
    ...
    private void print(Result result, Map<String, Object> args, final int model) {
        if (DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id] == null ||
                !DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getConnState()) {
        ...
        if (args.containsKey("config") && args.containsKey("data")) {
            ...
            threadPool.addSerialTask(new Runnable() {
                @Override
                public void run() {
                    PrinterCommand printModel = DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getCurrentPrinterCommand();
                    if (printModel == PrinterCommand.ESC) {
//                        if (printModel != PrinterCommand.ESC) {
//                            DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].sendByteDataImmediately(FactoryCommand.changPrinterMode(FactoryCommand.printerMode.ESC));
//                            try {
//                                Thread.sleep(3000);
...
//                        }
                    ...
                    } else if (printModel == PrinterCommand.TSC) {
//                        if (printModel != PrinterCommand.TSC) {
//                            DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].sendByteDataImmediately(FactoryCommand.changPrinterMode(FactoryCommand.printerMode.TSC));
//                            try {
//                                Thread.sleep(3000);
//                            } catch (InterruptedException e) {
    ...
    private final StreamHandler stateHandler = new StreamHandler() {
        ...
        private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                String extra = intent.getStringExtra(EXTRA_DEVICE);
                Log.d(TAG, "stateStreamHandler, current action: " + action + " extra: " + extra);
